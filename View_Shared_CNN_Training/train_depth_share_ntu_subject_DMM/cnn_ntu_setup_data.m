...
i = 1;
file_train = dir(fullfile(imdb.imageDir.train, 'S*'));%*.jpg
...
for d = 1:55:length(file_train)
    name{end+1} = [file_train(d).name(1:20),'.jpg'];%
...
i = 1;
file_test = dir(fullfile(imdb.imageDir.test, 'S*'));%*.jpg
...
for d = 1:55:length(file_test)
        name{end+1} = [file_test(d).name(1:20),'.jpg'];%
