...
% 保存特征到文件夹的版本
% 选择不同层的特征 + gmm fv + PCA + liblinear实验
%
clc
clear
...
for i = 1:length(index_train)
    ...
    if ~exist(['feature\',pic_name(1:end-4),'.mat'],'file')  
        ...
        data1 = net1.vars(net1.getVarIndex('x11')).value ;
        data1 = squeeze(gather(data1));
        data2 = net1.vars(net1.getVarIndex('x13')).value ;
        data2 = squeeze(gather(data2));
        data3 = net1.vars(net1.getVarIndex('x16')).value ;
        data3 = squeeze(gather(data3));
        data4 = net1.vars(net1.getVarIndex('x19')).value ;
        data4 = squeeze(gather(data4));
    
        data = {data1,data2,data3,data4};
        save(['feature\',pic_name(1:end-4),'.mat'],'data');
        clear data1 data2 data3 data4 data
    end
    
    temp = load(['feature\',pic_name(1:end-4),'.mat']);
    temp = temp.data;
    data_train{i} = [temp{1}(:)];%fc7
    train_label(i) = label;
    clear temp
end
...
%% fv
Mat_train = double(cell2mat(data_train));
...
c = [2^-6 2^-5 2^-4 2^-3 2^-2 2^-1 2^0 2^1 2^2 2^3 2^4];  
max_acc = 0;  
tic;  
for i = 1 : size(c, 2)  
