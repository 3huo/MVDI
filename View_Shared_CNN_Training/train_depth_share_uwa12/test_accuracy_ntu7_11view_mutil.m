...
% net_view1 = dagnn.DagNN.loadobj(load('F:\chenjun\dynamic\exp\uwa_DMM_view112\net-deployed-view1.mat')) ;
% net_view1.mode = 'test' ;
% net_view2 = dagnn.DagNN.loadobj(load('F:\chenjun\dynamic\exp\uwa_DMM_view212\net-deployed-view2.mat')) ;
% net_view2.mode = 'test' ;
% net_view3 = dagnn.DagNN.loadobj(load('F:\chenjun\dynamic\exp\uwa_DMM_view312\net-deployed-view3.mat')) ;
% net_view3.mode = 'test' ;
% net_view4 = dagnn.DagNN.loadobj(load('F:\chenjun\dynamic\exp\uwa_DMM_view412\net-deployed-view4.mat')) ;
% net_view4.mode = 'test' ;
% net_view5 = dagnn.DagNN.loadobj(load('F:\chenjun\dynamic\exp\uwa_DMM_view512\net-deployed-view5.mat')) ;
% net_view5.mode = 'test' ;

%all view is the same imdb
imdb = load('F:\chenjun\dynamic\exp\uwa_DMM_view112\imdb.mat') ;
...
for i = 1:length(index_train)
    ...
    if ~exist(['feature_DMM_uwa12\',pic_name(1:end-4),'.mat'],'file')  
        for view = 1:11
            data1 = [];
            data2 = [];
            for j = 1:5
                im_ =  imread(fullfile('F:\chenjun\dynamic\data\dynamic_multi_uwa3d_DMM',[pic_name(1:end-4),'_',num2str(view),'_',num2str(j),'.jpg']));
                im_ = single(im_);
                if view == 1 || view == 2
                    net_temp = net_view1;
        ...
        save(['feature_DMM_uwa12\',pic_name(1:end-4),'.mat'],'data');
        clear data
    end
    
    temp1 = load(['feature_DMM_uwa12\',pic_name(1:end-4),'.mat']);
    temp1 = temp1.data;
    for view = 1:11
