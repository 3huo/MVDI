...
% model_dir                   = fullfile(pwd, 'output', 'faster_rcnn_final', 'faster_rcnn_VOC0712_vgg_16layers'); %% VGG-16
 model_dir                   = fullfile(pwd, 'output_ntu', 'faster_rcnn_final', 'faster_rcnn_VOC2007_ZF'); %% ZF
...
%% -------------------- TESTING --------------------
im_names = {'Depth-00000025.png'};
% path = 'D:\myself\dataset\nturgb+d_depth';

% path = 'D:\myself\dataset\nut_depth2\nturgb+d_depth';
% choose = dir([path,'\S*']);
% nn = randperm(length(choose));
% for ii = 1:50
%     im_names{ii} = [path,'\',choose(nn(ii)).name,'\Depth-00000040.png'];
% end
% 
% path = 'D:\myself\dataset\nturgb+d_depth';
% choose = dir([path,'\S*']);
% 
% % for kk = 1:length(choose)
%     
% path_temp = [path,'\',choose(60).name];
% num_p = dir([path_temp,'\*.png']);
% for ii = 1:length(num_p)
%     im_names{ii} = [path_temp,'\',num_p(ii).name];
% end
