...
for j = 1:length(im_names)
    ...
    figure(j);
    imshow(im(:,:,1),[]);
    for box_num = 1:size(boxes_cell) 
        a = boxes_cell{box_num}(1);
        b = boxes_cell{box_num}(2);
        c = boxes_cell{box_num}(3)-boxes_cell{box_num}(1);
        d = boxes_cell{box_num}(4)-boxes_cell{box_num}(2);
