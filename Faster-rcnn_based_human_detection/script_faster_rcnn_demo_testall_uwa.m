function script_faster_rcnn_demo_testall_uwa()
close all;
clc;
clear mex;
...
% model_dir                   = fullfile(pwd, 'output', 'faster_rcnn_final', 'faster_rcnn_VOC0712_vgg_16layers'); %% VGG-16
 model_dir                   = fullfile(pwd, 'output_uwa', 'faster_rcnn_final', 'faster_rcnn_VOC2007_ZF'); %% ZF
...
%% -------------------- TESTING --------------------
num = 1;
path1 = 'F:\chenjun\dynamic\data\UWA3DII_Depth\ActionsNew';
path1_name = dir([path1,'\a*']);
for s = 1:length(path1_name)
    load([path1,'\',path1_name(s).name]);
    depth = curFrame;
    
    num
    num = num + 1;
    if exist(['all_bounding_box\',path1_name(s).name,'.mat'],'file')
        continue;
    end
    
    frame = 1;
    box_all = [];
    for n = 1:3:size(depth,3)
...
%         showboxes(im, boxes_cell, classes, 'default');%voc
    ...
    save(['all_bounding_box\',path1_name(s).name],'boxes_cell');
