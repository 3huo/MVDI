...
opts.dataDir = fullfile('data','UCLA_MBB') ;
opts.expDir_view1  = fullfile('exp_1_16', 'ucla_one_view1') ;
opts.expDir_view2  = fullfile('exp_1_16', 'ucla_one_view2') ;
opts.expDir_view3  = fullfile('exp_1_16', 'ucla_one_view3') ;
opts.expDir_view4  = fullfile('exp_1_16', 'ucla_one_view4') ;
opts.expDir_view5  = fullfile('exp_1_16', 'ucla_one_view5') ;
...
opts.imdbPath_view1 = fullfile(opts.expDir_view1, 'imdb.mat');
...
nCls = 12;
net_view1 = load(opts.modelPath);
net_view1 = prepareDINet(net_view1,nCls,opts);
...
% param_all = [];
% for layer = 1:16
%     param_all.net1.layer(layer).value = net_view1.params(layer).value;
%     param_all.net2.layer(layer).value = net_view2.params(layer).value;
%     param_all.net3.layer(layer).value = net_view3.params(layer).value;
%     param_all.net4.layer(layer).value = net_view4.params(layer).value;
%     param_all.net5.layer(layer).value = net_view5.params(layer).value;
% end
% -------------------------------------------------------------------------
%                                                              Prepare data
% -------------------------------------------------------------------------

if exist(opts.imdbPath_view1,'file')
  imdb_view1 = load(opts.imdbPath_view1) ;
  ...
  mkdir(opts.expDir_view1) ;
  save(opts.imdbPath_view1, '-struct', 'imdb') ;
...
imdb_view1.images.set = imdb_view1.images.sets;
...
net_view1.meta.classes.name = imdb_view1.classes.name ;
net_view1.meta.classes.description = imdb_view1.classes.name ;
...
imageStatsPath_view1 = fullfile(opts.expDir_view1, 'imageStats_view1.mat') ;
...
for index = 1:5
    ...
    for i = 1:length(train)-1
        ...
        temp = fn(imdb,index,batch(train(i)),aveimg) ;
        temp = temp{2};
        %     temp1 = mean(temp,4);
...
temp1 = gather(averageImage{1});
temp2 = gather(averageImage{2});
temp3 = gather(averageImage{3});
temp4 = gather(averageImage{4});
temp5 = gather(averageImage{5});

averageImage_view1 = (temp1+temp2+temp3+temp4+temp5)/5;
averageImage_view2 = (temp1+temp2+temp3+temp4+temp5)/5;
averageImage_view3 = (temp1+temp2+temp3+temp4+temp5)/5;
averageImage_view4 = (temp1+temp2+temp3+temp4+temp5)/5;
averageImage_view5 = (temp1+temp2+temp3+temp4+temp5)/5;
