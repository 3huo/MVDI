...
% 设置对应的训练样本集与输出模型的文件夹目录
% 输出模型的文件夹目录不要求一定存在，会自动生成
opts.dataDir = fullfile('data','ntu_view_MBB') ;
opts.expDir_view1  = fullfile('exp', 'ntu_shareview_view1') ;
opts.expDir_view2  = fullfile('exp', 'ntu_shareview_view2') ;
opts.expDir_view3  = fullfile('exp', 'ntu_shareview_view3') ;
opts.expDir_view4  = fullfile('exp', 'ntu_shareview_view4') ;
opts.expDir_view5  = fullfile('exp', 'ntu_shareview_view5') ;
% 加载预训练网络模型：采用的是vgg-f网络模型
opts.modelPath = fullfile('models','imagenet-vgg-f.mat');
[opts, varargin] = vl_argparse(opts, varargin) ;

opts.numFetchThreads = 12 ;
opts.lite = false ;
% 设置运行产生的重要结果文件：如均值，样本训练目录索引
...
% 
opts.train = struct() ;
opts.train.gpus = [3]; % 设置使用GPU端号,使用之前需查询是否可用
opts.train.batchSize = 8*3 ; % 设置batchSize
opts.train.numSubBatches = 4 ; % 设置子batchSize
opts.train.learningRate = 1e-4 * [ones(1,10), 0.1*ones(1,5)]; %设置学习率
...
for i = 1:length(batch)
    ...
    if imdb.images.set(batch(i)) == 1 %1为训锟斤拷锟斤拷锟斤拷锟侥硷拷锟斤拷
