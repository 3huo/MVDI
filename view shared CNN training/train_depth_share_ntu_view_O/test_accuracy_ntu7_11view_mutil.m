...
file = 'feature_2_1\feature_ntu_view_share_O';
if ~exist(file,'file')
    mkdir(file);
end
...
for i = 1:length(index_train)
    ...
    if ~exist([file,'\',pic_name(1:end-4),'.mat'],'file')  
        ...
        for view = 1:11
            ...
            data2 = [];
            for j = 1:5
                im_ =  imread(fullfile('F:\chenjun\dynamic\data\dynanic_multi_ntu_O\new\',[pic_name(1:end-4),'_',num2str(view),'_',num2str(j),'.jpg']));
                im_ = single(im_);
                if view == 1 || view == 2
                    net_temp = net_view1;
...
%         save([file,'\',pic_name(1:end-4),'_softmax.mat'],'datapre');
        clear data datapre
    end
...
for view = 1:length(viewchoose)
    tem = viewchoose(view);
    tem
    Tr_f = double(cell2mat(data_train(:,tem)'));
    Te_f = double(cell2mat(data_test(:,tem)'));
