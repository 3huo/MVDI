function imo = cnn_imagenet_get_batch_single(images, varargin)
...
imo = zeros(opts.imageSize(1), opts.imageSize(2), 11, ...
    numel(images)*opts.numAugments/11, 'single') ;

si = 1 ;
for ind = 1:11:numel(images)
    imtt = zeros(opts.imageSize(1), opts.imageSize(2), 11);
    for i = 1:11
        % acquire image
        if isempty(im{ind - 1 + i})
            imt = imread(images{i}) ;
    ...
    if ~isempty(opts.averageImage)
        ...
        imo(:,:,:,si) = bsxfun(@minus, imtt , offset) ;
    else
        imo(:,:,:,si) = imtt ;
    end
    si = si + 1 ;
